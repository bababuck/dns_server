...
/**
 * Process a TCP connection requests for a file transfer or update.
 *
 * @params coms: Coms object who's data will be used
 *
...
/**
 * Send entire host file via TCP.
 *
 * @params socket: Connected socket to send data over
 * @params coms: Coms object who's data will be sent
 *
 * @returns Error code, 0 if successful
 */
...
/**
 * Recieve an update to the host file
 *
 * @params coms: Coms object who's data will be sent
 * @params socket: Connected socket to send data over
 *
...
/**
 * Actually perform the host file update.
 *
 * @params coms: Coms object to update
 * @params remove: Should the item be remove, if false, add
 * @params domain: Domain name to be modified
 * @params ip: IP to translate too, if adding
 * @params id: Version number of transaction
 *
 * @returns Error code, 0 if successful
 */
...
/**
 * Function to run in thread to continually check for hosts.txt requests.
 *
...
uint8_t send_entire_file(int socket, coms_t *coms) {
  ...
  if (send(socket, &(coms->version_num), 1, 0) < 0) {
    return 5;
  }
  return 0;
}
