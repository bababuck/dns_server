...
int main(int argc, char **argv) {
  ...
  // Get the domains to request, add entry that will miss
...
void* add_hosts_fail_wrapper(void *arg) {
  ...
  char *ip = get_ip();
  int id = 0;
  int choice = (int) ((uint8_t) rand()) % (generator->dns_server_cnt + 1);
  if (choice == generator->dns_server_cnt)
    choice = -1;
  update_hosts(generator->dns_servers[0]->coms, ip, ip, false, "new_domain", "6.6.6.6", generator->dns_servers[0]->tcp_port_num, 16, choice);
...
uint8_t run_test(arguments_t *arguments) {
  ...
  else if (arguments->fail_translation_changes) {
    pthread_t *thread = malloc(sizeof(pthread_t));
    for (uint32_t i = 0; i < 0x8FF; i += 0x100) {
      for (uint32_t j = 0; j < 0x100; ++j) {
        send_single_test(generator, j + i);
